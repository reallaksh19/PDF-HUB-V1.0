...
export class PdfEditAdapter {
  ...
  static async insertImage(
    ...
    // Naively attempt to embed as PNG or JPG based on magic bytes/error fallback
    let image;
    try {
    ...
    for (const pageIndex of options.pages) {
      const page = pdfDoc.getPage(pageIndex);
      // coordinate system: y is from bottom
      const y = page.getHeight() - options.y - options.height;
